...
export function FlipCard({ children, backContent, className = "" }: FlipCardProps) {
  ...
  const shouldBeFlipped = animState === 'opening' || animState === 'open';
  ...
  return (
    <div className={`relative w-full aspect-square ${className}`} ref={slotRef}>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .flip-card-back p {
          font-size: calc(14px / var(--card-scale, 1)) !important;
          line-height: 1.5 !important;
        }
        @media (min-width: 768px) {
          .flip-card-back p {
            font-size: calc(16px / var(--card-scale, 1)) !important;
          }
        }
        .flip-card-back h1,
        .flip-card-back h2,
        ...
        @media (min-width: 768px) {
          ...
          .flip-card-back h3,
          .flip-card-back h4 {
            font-size: calc(18px / var(--card-scale, 1)) !important;
          }
        }
        .flip-card-back span {
          font-size: calc(9px / var(--card-scale, 1)) !important;
          line-height: 1.3 !important;
        }
      `}} />
      ...
      >
        <div className="flip-card-inner w-full h-full relative">
          ...
          {/* VERSO (DOS) */}
          <div className={`flip-card-back w-full h-full overflow-hidden bg-[#f7f3ea] text-black p-3 md:p-6 flex flex-col justify-between border border-[#CBBFA0]/20 relative transition-all duration-300 ${isActive ? "" : "group-hover:border-[#CBBFA0]/50 group-hover:bg-[#E5DCC3]/20"}`}>
      ...
      {mounted && createPortal(
        <div 
          ...
          style={{ 
            opacity: shouldBeFlipped ? 1 : 0, 
            pointerEvents: shouldBeFlipped ? 'auto' : 'none',
            zIndex: 40 
          }}
          onClick={shouldBeFlipped ? toggleFlip : undefined}
        />,
        document.body
      )}
    </div>
